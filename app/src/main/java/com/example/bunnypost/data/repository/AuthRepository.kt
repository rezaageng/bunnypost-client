...
class AuthRepository @Inject constructor(
    ...
    fun updateProfile(
        ...
        try {
            ...
            // FIX: Convert EditProfileRequest to Map<String, String> using toMap()
            val response = apiService.updateMyProfile("Bearer $token", userId, editProfileRequest.toMap())

            if (response.success) {
                response.data?.let { userData -> // userData ini harus sudah yang terbaru dari backend
                    val updatedUserEntity = UserEntity(
                        id = userData.id,
                        email = userData.email,
                        username = userData.username,
                        firstName = userData.firstName, // Pastikan ini sudah nilai baru
                        lastName = userData.lastName,   // Pastikan ini sudah nilai baru
                        profilePicture = userData.profilePicture,
                        bio = userData.bio
                    )
                    withContext(Dispatchers.IO) {
